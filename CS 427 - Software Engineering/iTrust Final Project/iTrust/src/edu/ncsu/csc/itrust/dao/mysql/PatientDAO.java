...
public class PatientDAO {
	...
	private PatientAllergyBeanLoader patientAllergyLoader;
	...
	public PatientDAO(DAOFactory factory) {
		...
		this.patientAllergyLoader = new PatientAllergyBeanLoader();
	}
	...
	public void deletePatient(PatientBean p, long hcpid) throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
	...
	/**
	 * Fetches a list of patients filtered by zip and allergy
	 */
	public List<PatientAllergyBean> getPatientAllergyList(String zip, String allergy) throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
			String stmt = "SELECT MID, lastName, firstName, zip, a.Description FROM patients p LEFT JOIN allergies a ON p.MID = a.PatientID WHERE MID > 0";
			if(StringUtils.isNotEmpty(zip)) {
				stmt += " AND zip LIKE '" + zip + "%'";
			}
			if(StringUtils.isNotEmpty(allergy)) {
				stmt += " AND Description LIKE '%" + allergy + "%'";
			}
			ps = conn.prepareStatement(stmt);
			ResultSet rs = ps.executeQuery();
			List<PatientAllergyBean> loadlist = patientAllergyLoader.loadList(rs);
			rs.close();
			ps.close();
			return loadlist;
		} catch (SQLException e) {

...
public class AddPatientAction {
	...
	public long addPreregisterPatient(PatientBean p, String pwd) throws FormValidationException, ITrustException {
		new AddPatientValidator().validate(p);
		long newMID = patientDAO.addEmptyPatient();
		p.setMID(newMID);
		p.setLastName(p.getLastName() + ".PREREGISTERED");
		authDAO.addUser(newMID, Role.PATIENT, pwd);
		p.setPassword(pwd);
		patientDAO.editPatient(p, loggedInMID);
		return newMID;
	}

	public void activatePreregisterPatient(PatientBean p) throws ITrustException {
		try {
			String name = p.getLastName().substring(0, p.getLastName().length() - 14);
			p.setLastName(name);
			patientDAO.editPatient(p, loggedInMID);
		}catch (IndexOutOfBoundsException e) {
			throw new ITrustException("Patient name does not include .PREREGISTERED suffix");
		}
	}

	public void deactivatePreregisterPatient(PatientBean p) throws ITrustException {
		try {
			String name = p.getLastName().substring(0, p.getLastName().length() - 14);
			p.setLastName(name);
			patientDAO.deletePatient(p, loggedInMID);
		}catch (IndexOutOfBoundsException e) {
			throw new ITrustException("Patient name does not include .PREREGISTERED suffix");
		}
	}

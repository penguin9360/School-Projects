...
void run_server(char *port) {
	...
	struct addrinfo ai, *el;
	
	memset(&ai, 0, sizeof(ai));
	ai.ai_family=AF_INET;
	ai.ai_socktype=SOCK_STREAM;
	ai.ai_flags=AI_PASSIVE;
	int get_add=getaddrinfo(NULL,port,&ai,&el);
	
	if(get_add!=0){
		perror(gai_strerror(get_add));
		freeaddrinfo(el);
		exit(1);  
	}
	
	if(bind(sock,el->ai_addr,el->ai_addrlen) != 0)	{
		perror(NULL);
		freeaddrinfo(el);
		exit(1);  
	}
	...
	while(endSession!=1){
		...
		int if_accept = accept(sock,NULL,NULL);
		if(if_accept == -1){
			perror(NULL);
			freeaddrinfo(el);
			exit(1);  

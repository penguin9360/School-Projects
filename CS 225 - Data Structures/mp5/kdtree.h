...
class KDTree
{
  ...
  private:
     ...
     */
     ...
     // ctor helper
     KDTreeNode * KDTreeHelper(vector<Point<Dim>>& points, int start, int end, int dist);
     // Dtor helper
     void clear(KDTreeNode * subRoot);
     // helper for findNearestNeighbor
     Point<Dim> findNearest_Helper(KDTreeNode* subRoot, const Point<Dim> tgt, Point<Dim> nearest, int newDim) const;
     // a helper used to calculate median
     void findMedian(vector<Point<Dim>>& points, int start, int end, int mid, int dist);
     // divide the image into regions
     int divide(vector<Point<Dim>>& points, int start, int end, int center, int dist);
     // swapping two points
     void swap(vector<Point<Dim>>& points, int index_1, int index_2);

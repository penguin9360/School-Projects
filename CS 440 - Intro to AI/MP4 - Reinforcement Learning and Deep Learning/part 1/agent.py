...
class Agent:
    ...
    def reset(self):
        self.points = 0
        self.s = None   #holds old atate and action
        self.a = None
    ...
    #1. Descritize states (get states index into Qtable)
       #top left for snake head to still be alive and go into is 40,40, top right is 480,40, bottom right is 480,480, bottom left i 40,480
        s_new=(2 if snake_head_x==480 else (1 if snake_head_x==40 else 0),
            2 if snake_head_y==480 else (1 if snake_head_y==40 else 0),
            ...
            1 if (snake_head_x,snake_head_y-40) in snake_body else 0,
            1 if (snake_head_x,snake_head_y+40) in snake_body else 0,
            1 if (snake_head_x-40,snake_head_y) in snake_body else 0,
            1 if (snake_head_x+40,snake_head_y) in snake_body else 0)
            
        # ik this looks silly but it's the only way i could access arrays
        sn1,sn2,sn3,sn4,sn5,sn6,sn7,sn8=s_new

    #2. Return best action if we're not training
        if not self.train:
            return self.actions[np.argmax(self.Q[sn1,sn2,sn3,sn4,sn5,sn6,sn7,sn8,:])]
    ...
    #4. Update the Q table with the LAST action that you took (not the one you are going to take) since the reward should be attributed to that action, not the next 
        if not (self.s==None and self.a==None): #this doesn't update Qtable if u are at first step (just resetted)
            ...
            a,b,c,d,e,f,g,h=self.s
            N=self.N[a,b,c,d,e,f,g,h,self.a]
            alpha = self.C/(self.C+N)
            #alpha = .8

            #update Q table
            Q=self.Q[a,b,c,d,e,f,g,h,:]
            #below should actually be the set of actions corresponding to this new state!
            Qs_next=np.max(self.Q[sn1,sn2,sn3,sn4,sn5,sn6,sn7,sn8,:])
            self.Q[a,b,c,d,e,f,g,h,self.a]+=alpha*(reward+self.gamma*Qs_next-self.Q[a,b,c,d,e,f,g,h,self.a])
    ...
    #TODO: Fix this exploration function i think    
        Ns=self.N[sn1,sn2,sn3,sn4,sn5,sn6,sn7,sn8,:]
        f=np.ones(len(self.actions))
        for action,freq in enumerate(Ns):
            if (freq>=self.Ne):
                f[action]=self.Q[sn1,sn2,sn3,sn4,sn5,sn6,sn7,sn8,action]
        #self.a=np.argmax(f)
    ...
    #8. update count of how many times we've seen that action and state 
        self.N[sn1,sn2,sn3,sn4,sn5,sn6,sn7,sn8,self.a]+=1

...
/**
 * Constructs a PronounceDict from a CMU map mapping the word
...
{
    /* Your code goes here! */
    string st1 = word1; string st2 = word2;
	transform(st1.begin(), st1.end(),st1.begin(), ::toupper);
	transform(st2.begin(), st2.end(),st2.begin(), ::toupper);
    auto gg1 = dict.find(st1);
    auto gg2 = dict.find(st2);

    if (gg1 == dict.end() or gg2 == dict.end() or gg1->second != gg2->second)
    	return 0;
    return 1;
}

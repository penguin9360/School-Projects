...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     while (singleQueue.size() + mergeQueue.size() > 1){
       TreeNode * p1 = removeSmallest(singleQueue, mergeQueue);
       TreeNode * p2 = removeSmallest(singleQueue, mergeQueue);
       TreeNode * newNode = new TreeNode(p1->freq.getFrequency() + p2->freq.getFrequency());
       newNode->left = p1;
       newNode->right = p2;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
     ...
     */
     ...
     if(bfile.getNextBit() == 1){
       ...
       newNode->left = HuffmanTree::readTree(bfile);
       newNode->right= HuffmanTree::readTree(bfile);
       return newNode;
     }
}

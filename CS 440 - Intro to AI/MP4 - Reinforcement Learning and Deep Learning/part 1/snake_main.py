...
class Application:
    ...
    def train(self):
        ...
        print("saved model",self.args.model_name)

    def test(self):
        print("Test Phase:")
        self.agent.eval()
        self.agent.load_model(self.args.model_name)
        print("loading model",self.args.model_name)
        points_results = []
        start = time.time()
    ...
    def show_games(self):
        ...
        pygame.event.pump()
        #self.agent.eval()
        #NOTE: CHANGING THIS TO WATCH TRAINING!
        self.agent.train()
        points_results = []
        end = False
        for game in range(1, self.args.show_eps + 1):
            ...
            while not dead:
                ...
                if keys[K_ESCAPE] or self.check_quit():
                    ...
                    break
                state, points, dead = self.env.step(action)
                #print("At state: ",state) #to see during human games 
                # snake_head_x, snake_head_y, snake_body, food_x, food_y = state
                
                # s=(2 if snake_head_x==480 else (1 if snake_head_x==40 else 0),
...
def main():
    ...
    parser.add_argument('--show_episodes', dest="show_eps", type=int, default=0, #was 10

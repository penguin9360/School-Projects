...
const std::chrono::milliseconds kForcesleepMs =
...
void Navigator::drive() {
  ...
  while (!ss_->done) {
		...
		// cout << "MOD == " << (cycles_since_last_forcesleep % kForcesleepCycleMod) << endl;

		// Every X cycles, send stop command and just sleep
		if (cycles_since_last_forcesleep % kForcesleepCycleMod == 0 && nav_state == NavState::STRAIGHT) {
    ...
    case NavState::BOOT:
      ...
      if (bump_left || bump_right) {
        ...
        nav_manager.sendDriveCommand(kFastSpeed,
                                     Create::DriveCommand::DRIVE_STRAIGHT, false);
        change = false;
      }

      if (DBG) { cout << "BOOT" << endl; }
      break;
    ...
    case NavState::STRAIGHT:
      ...
      if (bump_left || bump_right) {
        ...
        //nav_state = NavState::TURN_LEFT;
				...
				// If we are coming out a right turn and bumping, it's an align; else, we should turn
    ...
    case NavState::ALIGN_LEFT:
      ...
      if (DBG) { cout << "ALIGN_LEFT" << endl; }

      if (bump_left || bump_right) {
        // If the bump sensor is engaged, back up until it isn't (will we ever
        // encounter this?)
        nav_manager.sendDriveCommand(
            kBackupSpeed, Create::DriveCommand::DRIVE_STRAIGHT);
    		std::this_thread::sleep_for(kNavigationSleepMs * 2); // Give extra backup since this is a serious error ~ into the wall when aligning
				//nav_state = NavState::TURN_LEFT;  // Go into a turn left ~ should never bump unless misclassifying a turn as an align
				change = true;
        ...
        nav_manager.sendDriveCommand(
            kSlowSpeed, Create::DriveCommand::DRIVE_STRAIGHT);
    ...
    case NavState::TURN_RIGHT:
      ...
      if (bump_left || bump_right) {
        ...
        cycles_since_last_right = 0;
        nav_manager.sendDriveCommand(
            kAlignSpeed, Create::DriveCommand::DRIVE_INPLACE_CLOCKWISE);  // Track on this specific align, since the turn is 'over'
        nav_state = NavState::ALIGN_RIGHT;
        change = true;

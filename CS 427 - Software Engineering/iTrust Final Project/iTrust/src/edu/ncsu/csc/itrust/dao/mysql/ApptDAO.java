...
public class ApptDAO {
	...
	//uc 41
	public List<ApptBean> getApptsWithin(final long days) throws SQLException, DBException {
		Connection conn = null;
		PreparedStatement pstring = null;
		try{
			conn = factory.getConnection();
			pstring = conn.prepareStatement("SELECT * FROM appointment WHERE sched_date > CURDATE() AND HOUR(TIMEDIFF(sched_date, NOW())) <= ?  ORDER BY sched_date;"); //TODO DOES THIS WORK IDFK
			pstring.setLong(1, 24*days);
